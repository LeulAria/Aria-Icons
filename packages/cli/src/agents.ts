...
function getClaudeCodeConfigPath(home: string): string {
  ...
  if (existsSync(nativeConfigPath)) return nativeConfigPath;
...
export function getMcpServerConfig(remoteUrl?: string) {
  if (remoteUrl) {
    return {
      "aria-icons": {
        url: remoteUrl,
...
export function getOpenCodeMcpConfig(remoteUrl?: string) {
  if (remoteUrl) {
    return {
      "aria-icons": {
